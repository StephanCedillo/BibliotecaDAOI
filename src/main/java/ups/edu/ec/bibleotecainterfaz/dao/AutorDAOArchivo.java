...
public class AutorDAOArchivo implements AutorDAO {
    ...
    private static final int TAM_REGISTRO = (TAM_NOMBRE + TAM_APELLIDO) * 2; // 50 bytes
    ...
    public AutorDAOArchivo() {
        try {
            ...
            File archivo = new File(rutaDirecion, "autores.ups");
    ...
    public void crear(Autor autor) {
        try (RandomAccessFile archivo = new RandomAccessFile(new File(rutaDirecion, "autores.ups"), "rw")) {
    ...
    public Autor buscar(Autor autor) {
        try (RandomAccessFile archivo = new RandomAccessFile(new File(rutaDirecion, "autores.ups"), "r")) {
            ...
            for (int i = 0; i < totalRegistros; i++) {
                ...
                if (nombre.trim().equals(autor.getNombre()) && apellido.trim().equals(autor.getApellido())) {
    ...
    public boolean actualizar(Autor autorOriginal, Autor autor) {
        try (RandomAccessFile archivo = new RandomAccessFile(new File(rutaDirecion, "autores.ups"), "rw")) {
            ...
            for (int i = 0; i < totalRegistros; i++) {
                ...
                if (nombre.trim().equals(autorOriginal.getNombre()) && apellido.trim().equals(autorOriginal.getApellido())) {
                    archivo.seek(posicion);
                    archivo.writeChars(completarTexto(autor.getNombre(), TAM_NOMBRE));
                    archivo.writeChars(completarTexto(autor.getApellido(), TAM_APELLIDO));
                    return true;
                }
            }
        } catch (IOException e) {
    ...
    public boolean eliminar(Autor autor) {
        ...
        try (RandomAccessFile lectura = new RandomAccessFile(archivoOriginal, "r"); 
            ...
            for (int i = 0; i < totalRegistros; i++) {
                ...
                escritura.writeChars(completarTexto(nombre.trim(), TAM_NOMBRE));
                escritura.writeChars(completarTexto(apellido.trim(), TAM_APELLIDO));
            }
    ...
    public List<Autor> listar() {
        ...
        try (RandomAccessFile archivo = new RandomAccessFile(new File(rutaDirecion, "autores.ups"), "r")) {
            ...
            for (int i = 0; i < totalRegistros; i++) {
                ...
                lista.add(new Autor(nombre.trim(), apellido.trim()));
            }
    ...
    public void crearListadoTemporal(int cantidad) {
        // No aplica para archivos de acceso aleatorio
    }
